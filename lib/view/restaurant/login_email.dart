// ============================================================
// LOGIN WITH EMAIL — RESTAURANT SIDE — FIXED
// ============================================================
...
import 'package:forfood/view/verify_email_view.dart';  // ✅ Added for unverified redirect

class LoginWithEmailRestaurantView extends StatefulWidget {
  final UserRole role;

  const LoginWithEmailRestaurantView({super.key, required this.role});

  @override
  State<LoginWithEmailRestaurantView> createState() =>
      _LoginWithEmailRestaurantViewState();
}

class _LoginWithEmailRestaurantViewState
    extends State<LoginWithEmailRestaurantView> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<LoginBloc, LoginState>(
      listener: (context, state) {
        // Error SnackBar
        if (state is LoginStateError && state.generalError != null) {
          AppSnackbar.showError(context, state.generalError!);
        }

        // ✅ Redirect to VerifyEmailView if email not verified
        if (state is LoginStateError && state.isEmailNotVerified) {
          Navigator.push(
            context,
            fadeSlideRoute(
              VerifyEmailView(
        ...
        // Success snackbar (AuthBloc handles navigation)
                      ...
                      children: [
                        ...
                        Positioned(
                          ...
                          child: SizedBox(
                            ...
                            child: Text(
                              'List your menu on ForFood and reach thousands of customers every day..',
                              style: TextStyle(
                                color: AppColor.textDark,
